...
export default function PlayButton() {
  ...
  return (
    ...
    >
      ...
      {/* Play Icon */}
      <div className="absolute z-40 top-1/2 left-1/2 -translate-x-1/2 translate-y-full transition-all duration-300 group-hover:-translate-y-[40%]">
        <PlayCircle className="w-8 h-8 text-yellow-700 " />
      </div>
      
      {/* Background Wave Animation */}
      <svg
        className="absolute -mt-1 w-full h-full scale-x-125 rotate-180 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-30 group-hover:animate-none animate-pulse group-hover:-translate-y-[45%] transition-all duration-300"
      ...
      >
        ...
        >
          {[0, 35, 70, 105, 140, 175, 210, 245].map((translateY, index) => (
            <path
              key={translateY}
              opacity={((index + 1) * 0.16).toFixed(2)}
      ...
      {/* Second Wave */}
      <svg
        className="absolute w-full mt-1  h-full top-1/2 left-1/2 -translate-x-1/2 -translate-y-[30%] group-hover:-translate-y-[33%] group-hover:scale-95 transition-all duration-500 z-40 fill-yellow-700"
